#!/bin/bash
# =============================================================================
# Phase 3: Selective SLM Training
#
# Trains CosyVoice2's text-speech LM using Phase 2 token masks.
# Only top-k% speech tokens (by excess loss) contribute to training loss.
#
# Prerequisites:
#   - Phase 1 completed (RM checkpoint)
#   - Phase 2 completed (token_scores.pt)
#   - D_train parquet prepared (by Phase 2 script)
#
# Usage:
#   bash scripts/phase3_train.sh           # dry run
#   bash scripts/phase3_train.sh --run     # actual training
# =============================================================================
...
# Output paths
OUTPUT_DIR="${PROJECT_ROOT}/phase3_outputs"
...
echo "============================================================"
echo "Phase 3: Selective SLM Training"
echo "============================================================"
echo ""
echo "  Config:          ${CONFIG}"
...
for f in "${CONFIG}" "${MASK_PATH}" "${TRAIN_DATA_LIST}" "${CV_DATA_LIST}"; do
    if [ ! -f "${f}" ]; then
...
# ─── Generate Phase 3 config (patch LR / epoch) ─────────────────────────────

PHASE3_CONFIG="${OUTPUT_DIR}/phase3_cosyvoice2.yaml"
mkdir -p "${OUTPUT_DIR}" "${MODEL_DIR}" "${TENSORBOARD_DIR}"

# Reuse Phase 1 config but patch training hyperparams
...
print("Phase 3 config written to: ${PHASE3_CONFIG}")
CONFEOF
...
# Save command
CMD_FILE="${OUTPUT_DIR}/phase3_train_command.sh"
cat > "${CMD_FILE}" << CMD_INNER
#!/bin/bash
# Phase 3 training command (auto-generated)
...
if [ "${RUN_TRAINING}" = true ]; then
    echo "------------------------------------------------------------"
    echo "Launching Phase 3 selective training"
    echo "------------------------------------------------------------"
    ...
    echo "============================================================"
    echo "Phase 3 DRY RUN COMPLETE"
    echo "============================================================"
    echo ""
    echo "To launch training:"
    echo "  bash scripts/phase3_train.sh --run"
    echo ""
